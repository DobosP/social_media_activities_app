...
1. **Events JSON API is anonymous** — `EventViewSet` is `AllowAny`, routed through the same
   `upcoming_events()` / `events_with_public_places()` gates the public web pages, feeds and
   sitemap already use (parity, not new exposure).
...
4. **Search-engine dataset signals** — a public `/open-data/` page with schema.org `Dataset`
   ...
   (`/open-data/snapshot/<name>`, whitelist-only); `event_ld` enriched with `offers` /
   `isAccessibleForFree` from the RO-EDU price facts; `llms.txt` v2 documenting the
