...
- **`GROUPS_USER_CREATION_COHORTS` hard-wall**: CHILD/TEEN can **never** be added even by misconfig. (This wall stands on its own — the `CONNECTIONS_ALLOWED_COHORTS` hard-wall it originally mirrored was removed 2026-05-30, `78e5a48`; see `docs/adr/0002-cohort-connections-policy.md`.)
...
- **Notifications:** one new **mutable** `Notification.Kind.GROUP_ANNOUNCEMENT` (owner/staff broadcast) through the single `notify()` choke point + a `WHY_REASONS` entry + a no-op `makemigrations notifications` (convention — `ARCHITECTURE.md` "Working conventions"). Fan-out idempotent, blocked-pair-excluding, rate-limited. **Deliberately NO "X joined the group" / "people you may know" / member-growth notification** — that would be a roster-enumeration leak *and* engagement-maxxing (inv.2). MODERATION/SYSTEM stay non-mutable.
